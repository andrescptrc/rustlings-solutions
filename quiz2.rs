        ...
        for (string, command) in input.iter() {
            ...
            let transformed_string = match command {
                ...
                Command::Append(times) => {
                    let repeated = "bar".repeat(*times); // use the repeat method to create the repeated string
                    format!("{}{}", string, repeated) // use the format macro to concatenate the strings
                }
            };
